...
export const cv = {
  ...
  education: [
    "Systems Engineering, BEng (Hons), Loughborough University, 2007–2011",
  ],
  certifications: [
...
export function cvAsText(): string {
...
export function searchCv(query: string): Array<{ title: string; text: string }> {
  const terms = query
    .toLowerCase()
    .split(/[^a-z0-9.+#-]+/)
  ...
  return sections
    .map((section) => ({
      ...
      score: terms.reduce(
        (score, term) =>
          score +
          (`${section.title} ${section.text}`.toLowerCase().includes(term) ? 1 : 0),
        0
      ),
    }))
